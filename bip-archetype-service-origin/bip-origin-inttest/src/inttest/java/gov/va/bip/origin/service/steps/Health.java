...
public class Health {
	...
	public void getOriginHealthMessageValidation() {
		String status = JsonUtil.getString(handler.getStrResponse(), "status");
		String details = JsonUtil.getString(handler.getStrResponse(),
				"['details'].['Origin Service REST Endpoint']");
		assertThat(status, equalTo("UP"));
		assertThat(details, equalTo("Origin Service REST Provider Up and Running!"));
	}
