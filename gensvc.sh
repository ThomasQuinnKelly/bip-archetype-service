...
function show_help() {
	...
	echo "  https://github.com/department-of-veterans-affairs/ascent-archetype-service/"
	echo "* This script must be located in a directory that does not already" >&2
	echo "  contain the project to be generated." >&2
	echo "* A valid \"gensvc.properties\" file must exist in the same directory" >&2
...
echo "########################################################################" >&2
echo "#  Generate a BIP Service App project                          #" >&2
echo "########################################################################" >&2
echo "########################################################################" > $cwd/$generateLog 2>&1>&2
echo "#  Generate a VIP Service App project                          #" >> $cwd/$generateLog 2>&1
echo "########################################################################" >> $cwd/$generateLog 2>&1
get_args $args
read_properties
validate_properties
