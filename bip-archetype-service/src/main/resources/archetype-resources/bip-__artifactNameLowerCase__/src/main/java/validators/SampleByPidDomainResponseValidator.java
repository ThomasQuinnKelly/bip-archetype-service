...
public class SampleByPidDomainResponseValidator extends AbstractStandardValidator<SampleDomainResponse> {
	...
	public void validate(SampleDomainResponse toValidate, List<ServiceMessage> messages) {
		...
		if (toValidate == null || toValidate.getSampleInfo() == null) {
			${artifactName}MessageKeys key = ${artifactName}MessageKeys.BIP_SAMPLE_REQUEST_NOTNULL;
			LOGGER.info(key.getKey() + " " + key.getMessage());
			throw new ${artifactName}ServiceException(key, MessageSeverity.FATAL, HttpStatus.INTERNAL_SERVER_ERROR);
		}
		...
		if (!toValidate.getSampleInfo().getParticipantId().equals(request.getParticipantID())) {
			${artifactName}MessageKeys key = ${artifactName}MessageKeys.BIP_SAMPLE_REQUEST_PID_INCONSISTENT;
		...
		if (personTraits != null && StringUtils.isNotBlank(personTraits.getPid())) {
			if (toValidate.getSampleInfo() != null
				...
				${artifactName}MessageKeys key = ${artifactName}MessageKeys.BIP_SAMPLE_REQUEST_PID_INVALID;
				LOGGER.info(key.getKey() + " " + key.getMessage());
				toValidate.addMessage(MessageSeverity.WARN, HttpStatus.OK, key);
			}
		}
	 ...
	 * (non-Javadoc)
	 *
	 * @see gov.va.bip.framework.validation.AbstractStandardValidator${symbol_pound}setCallingMethod(java.lang.reflect.Method)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see gov.va.bip.framework.validation.AbstractStandardValidator${symbol_pound}getCallingMethod()
	 */
	@Override
